 ...
 */
function getPackageRoot(pModule: string) {
 ...
 *                           not be found
 */
function bareGetPackageJson(pModule: string, pBaseDir: string, pResolveOptions: Types.ResolveOption) {
  let lRetval = null;

  try {
    const lPackageJsonFilename = resolve(path.join(getPackageRoot(pModule), "package.json"), pBaseDir ? pBaseDir : ".", pResolveOptions);
...
const getPackageJson = _memoize(bareGetPackageJson, (pModule, pBaseDir) => `${pBaseDir}|${pModule}`);
 ...
 */
function dependencyIsDeprecated(pModule: string, pBaseDir: string, pResolveOptions: Types.ResolveOption) {
  let lRetval = false;
  const lPackageJson = getPackageJson(pModule, pBaseDir, pResolveOptions);
 ...
 */
function getLicense(pModule: string, pBaseDir: string, pResolveOptions: Types.ResolveOption) {
  let lRetval = "";
  const lPackageJson = getPackageJson(pModule, pBaseDir, pResolveOptions);

  if (Boolean(lPackageJson) && lPackageJson.hasOwnProperty("license") && typeof lPackageJson.license === "string") {
    lRetval = lPackageJson.license;
  }
  return lRetval;
}

export { getPackageRoot, getPackageJson, dependencyIsDeprecated, getLicense };
