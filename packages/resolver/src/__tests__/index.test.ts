...
describe("#resolve", () => {
  ...
  test("index.ts: native module: path", () => {
    ...
    const result = resolve({ moduleName: "path", moduleSystem: "cjs" }, baseDir, fileDir, options);
    expect(result.resolved).toBe("path");
  });

  test("index.ts: resolved-commonjs", () => {
    const { baseDir, fileDir } = generateAbsoluteDirPath("../index.ts");
    const result = resolve({ moduleName: "./resolved-commonjs", moduleSystem: "cjs" }, baseDir, fileDir, options);
    expect(result.resolved).toBe("src/resolved-commonjs.ts");
  });

  test("index.ts: @code-dependency/interfaces", () => {
    const { baseDir, fileDir } = generateAbsoluteDirPath("../index.ts");
    const result = resolve({ moduleName: "@code-dependency/interfaces", moduleSystem: "cjs" }, baseDir, fileDir, options);
    // lerna package
  ...
  test("resolve.ts: enhanced-resolve", () => {
    const { baseDir, fileDir } = generateAbsoluteDirPath("../resolve.ts");
    const result = resolve({ moduleName: "enhanced-resolve", moduleSystem: "cjs" }, baseDir, fileDir, options);
  ...
  test("determineDependencyTypes.ts: resolve", () => {
    ...
    const result = resolve({ moduleName: "resolve", moduleSystem: "cjs" }, baseDir, fileDir, options);
    // Reason: yarn workspace
